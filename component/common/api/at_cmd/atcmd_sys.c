...
ss_key_hash:
		...
		memset(key32, 0, sizeof(key32));
		sscanf_ret = sscanf(hash_default_key,
	...
	case 'd': {
		...
		if (argc > 3) {
			...
			if (valid_wake_pin(wake_pin)) {
				gpio_irq_init(&my_GPIO_IRQ, (PinName)PIN_NAME(PORT_A, wake_pin), NULL, (uint32_t)&my_GPIO_IRQ);
...
extern void uart_atcmd_reinit(UART_LOG_CONF *uartconf);
extern int write_uart_atcmd_setting_to_system_data(UART_LOG_CONF *uartconf);
...
void fATSG(void *arg)
{
	...
	if (argv[4]) {
		...
		gpio_dir(&gpio_ctrl->gpio[num], (PinDirection)dir);
	}
	if (argv[5]) {
		int pull = atoi(argv[5]);
		AT_DBG_MSG(AT_FLAG_GPIO, AT_DBG_ALWAYS, "PULL: %s", argv[5]);
		gpio_mode(&gpio_ctrl->gpio[num], (PinMode)pull);
	}
	if (argv[1][0] == 'R') {
