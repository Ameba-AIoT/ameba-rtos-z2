...
#if defined(MQTTV5)
		reasonCodes[(*count)++] = reasoncode
#else
		grantedQoSs[(*count)++]
#endif
		= readChar(&curdata);
#if defined(MQTTV5)
		mqtt_printf(MQTT_INFO,"%s ReasonCode: %d, %s",MQTTPacketIDPrint(header.bits.type),reasoncode,MQTTGetReason(reasoncode));
#endif
		}
	}
	rc = 1;
