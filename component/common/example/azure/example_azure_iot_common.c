	...
	iot_sample_variables *az_vars)
{
	...
	if (type == AZIOT_HUB) {
		...
		if (value) {
			IOT_SAMPLE_LOG("IOT_SAMPLE_HUB_HOSTNAME: %s", value);
		} else {
		...
		case AZIOT_872XD_PNP_SAMPLE:
			...
			if (value) {
				IOT_SAMPLE_LOG("IOT_SAMPLE_HUB_DEVICE_ID: %s", value);
			} else {
			...
			if (value) {
				IOT_SAMPLE_LOG("IOT_SAMPLE_DEVICE_X509_CERTIFICATE: \n%s", value);
			} else {
			...
			if (value) {
				IOT_SAMPLE_LOG("IOT_SAMPLE_DEVICE_X509_PRIVATE_KEY: \n%s", value);
			} else {
		...
		case AZIOT_872XD_PNP_SAS_SAMPLE:
			...
			if (value) {
				IOT_SAMPLE_LOG("IOT_SAMPLE_HUB_SAS_KEY: %s", value);
			} else {
			...
			if (az_result_succeeded(rc)) {
				IOT_SAMPLE_LOG("IOT_SAMPLE_SAS_KEY_DURATION_MINUTES: %d", az_vars->sas_key_duration_minutes);
			} else {
		...
		if (value) {
			IOT_SAMPLE_LOG("IOT_SAMPLE_PROVISIONING_ID_SCOPE: %s", value);
		} else {
		...
		case AZIOT_872XD_PNP_SAMPLE:
			...
			if (value) {
				IOT_SAMPLE_LOG("IOT_SAMPLE_PROVISIONING_REGISTRATION_ID: %s", value);
			} else {
		...
		case AZIOT_872XD_PNP_SAS_SAMPLE:
			...
			if (value) {
				IOT_SAMPLE_LOG("IOT_SAMPLE_PROVISIONING_SAS_KEY: %s", value);
			} else {
