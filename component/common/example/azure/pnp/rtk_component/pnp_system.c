...
static az_span const twin_reported_processor_architecture_property_value =
	AZ_SPAN_LITERAL_FROM_STR("32-bit 200MHz ArmRCortexR-M4 & 32-bit 20MHz ArmRCortexR-M0");
	...
	az_span *out_response)
{
	...
	for (i = 0, str_counter = 0; i < 4; i++, str_counter++) {
		slice_buf[i] = time_buf[str_counter];
	}
	slice_buf[i] = '\0';
	rebootTime.tm_year = atoi(slice_buf) - 1900;
	str_counter++; // skip "-"

	for (i = 0; i < 2; i++, str_counter++) {
		slice_buf[i] = time_buf[str_counter];
	}
	slice_buf[i] = '\0';
