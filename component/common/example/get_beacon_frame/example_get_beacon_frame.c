...
int get_beacon_frame_func(BEACON_INFO_T beacon_frame)
{
	...
	printf("\nbeacon timestamp   = %02x %02x %02x %02x %02x %02x %02x %02x\n", \
		   beacon_frame.timestamp[0], beacon_frame.timestamp[1], beacon_frame.timestamp[2], beacon_frame.timestamp[3], beacon_frame.timestamp[4],
		   beacon_frame.timestamp[5], beacon_frame.timestamp[6], beacon_frame.timestamp[7]);
...
#endif //#if defined(CONFIG_EXAMPLE_GET_BEACON_FRAME) && CONFIG_EXAMPLE_GET_BEACON_FRAME