	...
	if (ret == LFS_ERR_OK) {
		for (;;) {
			...
			if (info.type == LFS_TYPE_DIR) {
				...
				printf("[Delete file] name: %s, type: %d, size: %d\r\n", info.name, info.type, info.size);
...
void example_littlefs_thread(void *param)
{
	...
	printf("Create a file and open it as write only: %s\r\n", file_name);
	...
	printf("Open a file as read only: %s\r\n", file_name);
	...
	printf("Avaliable space: %d, number of allocated blocks: %d\r\n", available, _df_nballocatedblock);
...
#endif //#if defined(CONFIG_EXAMPLE_LITTLEFS) && CONFIG_EXAMPLE_LITTLEFS