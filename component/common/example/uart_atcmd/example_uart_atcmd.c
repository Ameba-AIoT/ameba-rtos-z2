...
extern uint32_t sys_update_ota_get_fw1_sn(void);
...
int reset_uart_atcmd_setting(void)
{
#if 0
	flash_t flash;
...
static void uart_atcmd_thread(void *param)
{
	...
	uint32_t curr_fw_sn = 0;
	p_wlan_init_done_callback = NULL;

	uart_atcmd_main();
	curr_fw_idx = sys_update_ota_get_curr_fw_idx();
	if (curr_fw_idx == 1) {
		curr_fw_sn = sys_update_ota_get_fw1_sn();
	} else if (curr_fw_idx == 2) {
		curr_fw_sn = sys_update_ota_get_fw2_sn();
	}
	at_printf("\n\rCurrent firmware serial number is %d\r\n", curr_fw_sn);
