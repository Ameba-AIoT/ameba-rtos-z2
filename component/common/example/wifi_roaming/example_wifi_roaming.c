		...
		if (wifi_is_up(RTW_STA_INTERFACE) && (RTW_SUCCESS == wifi_is_ready_to_transceive(RTW_STA_INTERFACE))) {
			...
			if ((ap_rssi < RSSI_THRESHOLD)) {
				if (polling_count >= (MAX_POLLING_COUNT - 1)) {
					...
					wifi_scan_networks_with_ssid((int (*)(char *, int, char *, void *))wifi_roaming_find_ap_from_scan_buf, (void *)&roaming_ap, SCAN_BUFLEN,
												 (char *)roaming_ap.ssid, strlen((char const *)roaming_ap.ssid));
...
connect_ap:
				...
				polling_count = 0;
				printf("Current RSSI Threshold: %d, Current RSSI measured: %d\n", RSSI_THRESHOLD, ap_rssi);
				printf("Roaming will not be triggered with current RSSI Threshold setting\n");
			}
		}
		vTaskDelay(2000);// 2s
	}
...
#endif //#if defined(CONFIG_EXAMPLE_WIFI_ROAMING) && CONFIG_EXAMPLE_WIFI_ROAMING