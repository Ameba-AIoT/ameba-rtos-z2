...
::	copy %libdir%\image_z2plus\bootloader.bin %cfgdir%\Exe\bootloader.bin
::)
...
if not exist %libdir%\image_z2plus\bootloader.out (
	echo bootloader.out is missing
	echo bootloader.out is missing > postbuild_is_error.txt
	goto error_exit
)
copy %libdir%\image_z2plus\bootloader.out %cfgdir%\Exe\bootloader.out
